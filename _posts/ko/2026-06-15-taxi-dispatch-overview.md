...
seriesOrder: 1
description: "우버 같은 택시 배차 시스템은 어떻게 구현했을까. 화려한 분산 아키텍처부터 떠올리기 쉽지만, 아마 시작은 서버 하나·DB 하나였을 것이다. 이 시리즈 1편은 배차 서비스의 가장 단순한 전체 그림을 그리고, 트래픽이 늘면 어디서부터 문제가 될지(=앞으로 다룰 목록)만 적어둔다."
---

**우버 같은 배차 시스템은 어떻게 구현했을까.** "실시간으로 움직이는 차 수백만 대" 같은 말을 들으면 처음부터 거창한 분산 시스템을 떠올리게 된다. 그런데 시작부터 그랬을 리는 없다. 어떤 서비스든 첫 버전은 보통 서버 하나, DB 하나에서 출발한다. 이 시리즈는 그 단순한 그림에서 시작해서, 트래픽이 늘 때마다 문제가 되는 곳을 하나씩 손보는 순서로 가본다. 1편에서는 일단 배차 서비스 전체를 최대한 단순하게 그려본다.
