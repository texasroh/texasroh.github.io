...
3. **환자 코호트 평가 — 백엔드 Module Evaluator + 자동 비동기 다중 실행**
   모듈 정의를 백엔드(Python)에서 평가 / web-app 결과와 완전히 동일 보장 /
...
- 임상 모듈은 web-app(JS) 클라이언트에서만 돌아감 → 자동화·일괄 처리 불가능.
...
- web-app(JS) 평가 결과와 **완전히 동일** — 빌더 미리보기와 실제 환자 결과가 갈라지면 의료 안전 사고로 직결.
...
- **클라이언트 의존 제거** — 모듈이 web-app뿐 아니라 백엔드 어디서든 동일 결과로 실행.
- **외부 장애 격리** — EHR API 장애가 Lambda에서 소진, Django API는 영향 없음.
- **모듈 → 인구 보건 도구로 격상** — 같은 모듈이 1명 진료, N명 코호트 분석에 동시 활용.
...
- **위 세 축의 운영 기반** — LLM 플랫폼·워커 분리·코호트 평가 모두 서비스별 독립 배포·스케일·보안 격리를 전제로 함. EB로는 이 전제를 못 채움.
